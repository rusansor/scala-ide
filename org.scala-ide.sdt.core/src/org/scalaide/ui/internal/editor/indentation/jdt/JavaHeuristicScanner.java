package org.scalaide.ui.internal.editor.indentation.jdt;
/**
 * This is a copy of [[org.eclipse.jdt.internal.ui.text.JavaHeuristicScanner]], which can't
 * be used because it triggers UI bundles.
 */
...
public final class JavaHeuristicScanner implements Symbols {
  ...
  public boolean looksLikeClassInstanceCreationBackward(int start, int bound) {
    ...
    if (token == TokenIDENT) { // type name
      token= previousToken(getPosition(), bound);
      while (token == TokenOTHER) { // dot of qualification
        token= previousToken(getPosition(), bound);
        if (token != TokenIDENT) // qualification name
          return false;
        token= previousToken(getPosition(), bound);
      }
